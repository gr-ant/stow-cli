...
**Do not edit** any file under `stw/` that is not in your assigned list, and do
not edit `stw/cli.py` (every command is already registered) or `plan.md`.

## Environment

- `./bin/stw <args>` runs the CLI (uses `.venv`). Also `.venv/bin/python -m stw`.
...
Every file in `stw/commands/` exposes exactly:

```python
def add_arguments(parser: argparse.ArgumentParser) -> None: ...
def run(ws, conn, args) -> int: ...   # 0 on success; raise stwError otherwise
```
...
`stwError` subclass — `cli.main` renders it to stderr and picks the exit code.

## Foundation API

`stw/workspace.py`
- `Workspace.find(start) -> Workspace` (raises `NoWorkspace`), `Workspace.at(root)`
- `.root`, `.stw_dir`, `.index_path`, `.objects_dir`, `.config` (merged dict)
...
`stw/db.py` — `connect(path, create=False)`, `ensure_schema(conn)`, `tx(conn)`
...
`stw/out.py` — `emit(human, data)`, `line`, `raw`, `warn(code, msg, hint)`,
...
`stw/errors.py` — `stwError(code, message, hint)` plus the named subclasses in
plan.md §11. Add new subclasses there only if you own that file.

`stw/chunker.py` — `Chunk`, `chunk_document(path, text, cfg_chunk, headings)`,
...
`stw/hashing.py` — `sha256_bytes`, `sha256_text`, `short`.
